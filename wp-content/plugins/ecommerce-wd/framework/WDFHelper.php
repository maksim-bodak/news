  ...
  public static function get_order($order_id = 0) {
    ...
    if ($order->product_rows) {
      ...
      foreach ($order->product_rows as $product) {
        ...
        ob_start();
        if (is_array(json_decode($product->tax_info)) && $options->tax_total_display == 'itemized') {
    ...
    if (is_array($order_product_rows)) {
      foreach ($order_product_rows as $order_product_row) {
        ...
        if ($shipping_method_row) {
          ...
          $order_product_row->shipping_price_text = $options->option_order_shipping_type == 'per_order' ? '' : WDFHelper::price_text($current_shipping->price, $decimals, $row_default_currency);

          $shipping_count = $shipping_method_row->shipping_type == 'per_unit' ? $order_product_row->count : 1;
          $shipping_price = 0;
          if ($shipping_method_row->free_shipping == 1 || ($shipping_method_row->free_shipping == 2 && $order_product_row->subtotal >= $shipping_method_row->free_shipping_start_price)) {
            $shipping_price = 0;
          }
          else {
