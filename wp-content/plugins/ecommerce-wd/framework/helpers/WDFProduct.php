...
class WDFProduct {
  ...
  */
  ...
  public static function get_product_shippings($product_data, $checkout_data = array()) {
    ...
    // $product_shipping_classes = (array) $product_shipping_classes;
    if ($shipping_classes_with_price) {
      foreach ($shipping_classes_with_price as $key => $shipping_class_with_price) {
        if (isset($product_shipping_classes[$key]) && in_array($key, explode(',', $product_shipping_classes))) {
          // Available shipping classes ids.
          array_push($shipping_class_ids, $key);
          // Available shipping classes costs.
          $shipping_class_prices[$key] = $shipping_class_with_price;
          // Add available shipping classes costs.
          $shipping->price += $shipping_class_with_price;
        }
      }
    }
    // Free shipping.
