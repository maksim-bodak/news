...
while ( $r->have_posts() ) :
    ...
    if ( $post_counter == 1 ) {
        ...
        $image                  = $image_attr->get_image_attr_top_grid_10('first'. $mip_top_grid->top_grid_full_width);
        ...
        $image_attr             = new MipThemeFramework_Image();
        $image                  = $image_attr->get_image_attr_top_grid_10('second'. $mip_top_grid->top_grid_full_width);
...
echo    '<div class="col-md-8">'. $output_left .'</div><div class="col-md-4">'. $output_right .'</div>';
