...
while ( $r->have_posts() ) :
    ...
    if ( $post_counter < 4 ) {
        ...
        $image                  = $image_attr->get_image_attr_top_grid_5('first'. $mip_top_grid->top_grid_full_width);
        ...
        $output_top     .= '<div class="col-md-4">'. $post_article->formatArticleOverlay('h2', $mip_top_grid->top_grid_show_category, $mip_top_grid->top_grid_show_date, $mip_top_grid->top_grid_show_comments, $mip_top_grid->top_grid_show_author, $mip_top_grid->top_grid_show_views, 0, $post_counter) .'</div>';
    } else {
        $image_attr             = new MipThemeFramework_Image();
        $image                  = $image_attr->get_image_attr_top_grid_5('second'. $mip_top_grid->top_grid_full_width);
        ...
        $output_bottom     .= '<div class="col-md-3">'. $post_article->formatArticleOverlay('h3', $mip_top_grid->top_grid_show_category, $mip_top_grid->top_grid_show_date, $mip_top_grid->top_grid_show_comments, $mip_top_grid->top_grid_show_author, $mip_top_grid->top_grid_show_views, 0, $post_counter) .'</div>';
    }
