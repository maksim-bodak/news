...
if ( ! function_exists( 'breadcrumbs_plus' ) ) {
	function breadcrumbs_plus( $args = '' ) {
		...
		$defaults = array(
			...
			'suffix' => '',
			'title' => '',
			'home' => esc_html__( 'Home', 'Newsgamer' ),
			'separator' => ' ',
			'front_page' => false,
			'show_blog' => false,
		...
		if ( !$args['echo'] )
			...
			echo mipthemeframework_get_string_prefix() . $breadcrumbs;
	}
}
	...
	function breadcrumbs_plus_get_items( $args ) {
		...
		/* If viewing any type of archive. */
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				if ( is_day() )
					$item['last'] = esc_html__( 'Archives for ', 'Newsgamer' ) . get_the_time( MIPTHEME_DATE_DEFAULT );

				elseif ( is_month() )
					$item['last'] = esc_html__( 'Archives for ', 'Newsgamer' ) . single_month_title( ' ', false );

				elseif ( is_year() )
					$item['last'] = esc_html__( 'Archives for ', 'Newsgamer' ) . get_the_time( 'Y' );
			}

			else if ( is_author() )
				$item['last'] = esc_html__( 'Archives by: ', 'Newsgamer' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
		}

		/* If viewing search results. */
		else if ( is_search() )
			$item['last'] = esc_html__( 'Search results for "', 'Newsgamer' ) . stripslashes( strip_tags( get_search_query() ) ) . '"';

		/* If viewing a 404 error page. */
		else if ( is_404() )
			$item['last'] = esc_html__( 'Page Not Found', 'Newsgamer' );
	...
	function breadcrumbs_plus_get_bbpress_items( $args = array() ) {
		...
		elseif ( bbp_is_single_topic() ) {
			...
			if ( bbp_is_topic_split() )
				$item[] = esc_html__( 'Split', 'Newsgamer' );

			elseif ( bbp_is_topic_merge() )
				$item[] = esc_html__( 'Merge', 'Newsgamer' );

			elseif ( bbp_is_topic_edit() )
				$item[] = esc_html__( 'Edit', 'Newsgamer' );
		}
